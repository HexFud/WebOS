...
export function WindowFrame({windowItem: windowItem, state: state, dispatch: dispatch, openApp: openApp, showToast: showToast, openDesktopItem: openDesktopItem}) {
  ...
  const style = {
    ...
    zIndex: windowItem.z,
    "--spawn-x": windowItem.origin ? `${windowItem.origin.x - live.x}px` : "0px",
    "--spawn-y": windowItem.origin ? `${windowItem.origin.y - live.y}px` : "0px"
  };
  return h("section", {
    ref: frameRef,
    ...
    onPointerDown: event => startWindowDrag(event, windowItem, dispatch, frameRef, liveRef, interactingRef),
    ...
    icon: windowItem.maximized ? "restore" : "maximize"
  }))), h("div", {
    className: "window-title-group"
  }, h("div", {
    className: "window-title-icon"
  }, h(Icon, {
    icon: APPS[windowItem.appKey]?.icon || "folder"
  })), h("div", {
    className: "window-title"
  }, windowItem.title))), h("div", {
    className: "window-body"
  }, renderApp(windowItem, state, dispatch, openApp, showToast, openDesktopItem)), !windowItem.maximized && h(Fragment, null, ...RESIZE_EDGES.map(edge => h("div", {
    key: edge,
...
export function startWindowDrag(event, windowItem, dispatch, frameRef, liveRef, interactingRef) {
  ...
  const finish = endEvent => {
    ...
    const finalBounds = {
      x: originX + ((endEvent?.clientX ?? startX) - startX),
      y: originY + ((endEvent?.clientY ?? startY) - startY),
      width: originW,
      height: originH
    };
...
export function startWindowResize(event, edge, windowItem, dispatch, frameRef, liveRef, interactingRef) {
  ...
  const finish = endEvent => {
    ...
    window.removeEventListener("pointercancel", finish);
    window.removeEventListener("blur", finish);
    const bounds = computeBounds(endEvent?.clientX ?? startX, endEvent?.clientY ?? startY);
    liveRef.current = bounds;
    interactingRef.current = false;
    dispatch({
      type: "MOVE_WINDOW",
