...
import { BOOT_MS, UNLOCK_MS, NOTE_TEXT, MIN_PASSWORD_LENGTH, TASKBAR_HEIGHT } from "./lib/constants.js";
...
import { MissionControl } from "./components/mission-control.js";

import { STICKY_COLORS } from "./components/sticky-note.js";
...
export function App() {
  ...
  useEffect(() => {
    ...
    document.body.style.setProperty("--wallpaper-accent", resolveWallpaper(state.wallpaper, state.customWallpaper).accent);
  }, [ state.phase, state.theme, state.wallpaper, state.customWallpaper ]);
  useEffect(() => {
    const clock = window.setInterval(() => dispatch({
      type: "SET_CLOCK",
      value: new Date
    }), 1e3);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = event => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") {
        ...
        return dispatch({
          type: state.spotlightOpen ? "CLOSE_SPOTLIGHT" : "OPEN_SPOTLIGHT"
        });
      }
      ...
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "a") {
        ...
        event.preventDefault();
        return dispatch({
          type: "SET_DESKTOP_SELECTION",
          value: state.filesystem.children.filter(child => child.showOnDesktop).map(child => child.id)
        });
      }
      if ((event.key === "Delete" || event.key === "Backspace") && state.selectedDesktopItemIds.length) {
        if (["INPUT", "TEXTAREA"].includes(document.activeElement?.tagName)) return;
        event.preventDefault();
        state.selectedDesktopItemIds.filter(id => id !== "trash").forEach(id => dispatch({
          type: "DELETE_DESKTOP_ITEM",
          id: id
        }));
        return;
      }
      if (event.key === "Escape") {
        if (state.missionControlOpen) return dispatch({
          type: "CLOSE_MISSION_CONTROL"
        });
        if (state.spotlightOpen) return dispatch({
          type: "CLOSE_SPOTLIGHT"
        });
        if (state.notificationCenterOpen) return dispatch({
          type: "CLOSE_NOTIFICATION_CENTER"
        });
        if (state.controlCenterOpen) return dispatch({
          type: "CLOSE_CONTROL_CENTER"
        });
        if (state.appleMenuOpen) return dispatch({
          type: "CLOSE_APPLE_MENU"
        });
        if (state.contextMenu) return dispatch({
          type: "CLOSE_CONTEXT_MENU"
        });
        if (state.selectedDesktopItemIds.length) return dispatch({
          type: "SET_DESKTOP_SELECTION",
          value: []
        });
  ...
  }, [ state.activeWindowId, state.appleMenuOpen, state.contextMenu, state.controlCenterOpen, state.filesystem, state.missionControlOpen, state.notificationCenterOpen, state.phase, state.selectedDesktopItemIds, state.spotlightOpen, state.windows ]);
  useEffect(() => {
    if (!state.notificationCenterOpen && !state.controlCenterOpen && !state.appleMenuOpen && !state.contextMenu) return undefined;
    const closeMenus = () => dispatch({
      type: "CLOSE_OVERLAYS"
    });
  ...
  const openApp = (appKey, extras = {}) => {
    ...
    dispatch({
      type: "OPEN_WINDOW",
      appKey: appKey,
      bounds: buildWindowBounds(appKey, state.windows.length, extras.originRect),
      payload: extras
    });
  };
  ...
  const onDesktopAction = (action, value) => {
    ...
    if (action === "new-file") {
      const item = {
        id: uid("file"),
        type: "file",
        name: `New file ${state.filesystem.children.filter(child => child.type === "file").length + 1}.txt`,
        icon: "file",
        showOnDesktop: true,
        content: "New file created inside WebOS."
      };
      dispatch({
        type: "ADD_DESKTOP_ITEM",
        item: item
      });
    ...
    if (action === "new-folder") {
      const item = {
        id: uid("folder"),
        type: "folder",
        name: `New Folder ${state.filesystem.children.filter(child => child.type === "folder").length + 1}`,
        icon: "folder",
        showOnDesktop: true,
        children: []
      };
    ...
    if (action === "new-sticky") {
      const spawnX = clamp(state.contextMenu?.x ?? 220, 0, window.innerWidth - 220);
      const spawnY = clamp(state.contextMenu?.y ?? 160, 40, window.innerHeight - TASKBAR_HEIGHT - 200);
      dispatch({
        type: "ADD_STICKY_NOTE",
        note: {
          id: uid("sticky"),
          text: "",
          color: STICKY_COLORS[state.stickyNotes.length % STICKY_COLORS.length],
          x: spawnX,
          y: spawnY
        }
      });
    ...
    if (action === "set-wallpaper") {
      ...
      showToast("Wallpaper changed", resolveWallpaper(value, state.customWallpaper).label);
    }
    if (action === "sort-icons") {
      dispatch({
        type: "SORT_DESKTOP_ICONS"
      });
      showToast("Desktop sorted", "Icons were arranged in alphabetical order.");
    }
    if (action === "refresh") {
      dispatch({
        type: "RESET_DESKTOP_ICONS"
      });
  ...
  const onLockScreen = () => {
    ...
    showToast("Screen locked", "Enter your password to continue.");
  };
  const onRestart = () => dispatch({
    type: "RESTART_WEBOS"
  });
  const onAbout = () => {
    dispatch({
  ...
  const launchSpotlightItem = result => {
    ...
    if (item.type === "folder") return openApp("explorer", {
      path: [ "root", item.id ]
    });
    if (item.type === "file") return openApp("notes", {
      text: item.content || NOTE_TEXT,
      fileId: item.id
    });
  };
  return h("div", {
    className: `shell shell--${state.theme} shell--${state.phase}${state.unlocking ? " shell--unlocking" : ""}`
  }, h("div", {
    className: "wallpaper-backdrop",
    style: {
      backgroundImage: resolveWallpaper(state.wallpaper, state.customWallpaper).background,
      backgroundSize: "cover",
      backgroundPosition: "center"
    }
  }), state.phase !== "boot" && h(Desktop, {
    state: state,
    dispatch: dispatch,
    openApp: openApp,
    showToast: showToast
  }), state.phase === "boot" && h(BootScreen, {
    ...
    onPasswordChange: value => dispatch({
      type: "SETUP_PASSWORD_CHANGED",
      value: value
    }),
    onConfirmChange: value => dispatch({
      type: "SETUP_PASSWORD_CONFIRM_CHANGED",
      value: value
    }),
  ...
  }), state.phase === "desktop" && h(MenuBar, {
    state: state,
    activeAppTitle: activeAppTitle,
    dispatch: dispatch
  }), state.phase === "desktop" && h(Fragment, null, h(Taskbar, {
    state: state,
    openApp: openApp,
    dispatch: dispatch
  }), h(ContextMenu, {
    ...
    onAction: onDesktopAction
  })), state.appleMenuOpen && h(AppleMenu, {
    onLock: onLockScreen,
    onRestart: onRestart,
    onAbout: onAbout
  }), state.controlCenterOpen && h(ControlCenter, {
    state: state,
    dispatch: dispatch,
    onClose: () => dispatch({
      type: "CLOSE_CONTROL_CENTER"
    })
  }), state.spotlightOpen && h(Spotlight, {
    items: spotlightItems,
    onSelect: launchSpotlightItem,
    onClose: () => dispatch({
      type: "CLOSE_SPOTLIGHT"
    })
  }), state.notificationCenterOpen && h(NotificationCenter, {
  ...
  }), state.missionControlOpen && h(MissionControl, {
    windows: state.windows,
    onSelect: id => {
      const target = state.windows.find(window => window.id === id);
      if (target?.minimized) dispatch({
        type: "RESTORE_WINDOW",
        id: id
      }); else dispatch({
        type: "FOCUS_WINDOW",
